...
public class BrowserStackSerenityDriver implements DriverSource {
  ...
  public WebDriver newDriver() {
    ...
    while (it.hasNext()) {
      ...
      if (key.equals("browserstack.user")
          || key.equals("browserstack.key")
          || key.equals("browserstack.server")) {
        continue;
      } else if (key.startsWith("bstack_")) {
        capabilities.setCapability(
            key.replace("bstack_", ""), environmentVariables.getProperty(key));
      ...
      return new RemoteWebDriver(
          new URL(
              "https://"
                  ...
                  + accessKey
                  + "@"
                  + environmentVariables.getProperty("browserstack.server")
                  + "/wd/hub"),
          capabilities);
    } catch (Exception e) {
